...
@section('content')
    ...
    <div class="reservation__wrap">
        @if(!empty($today_date))
        <form class="reservation__content" action="/reservation" method="post">
        @else
        <form class="reservation__content" action="{{ request()->is('*edit*') ? route('reservation.update', $reservation) : route('reservation', $shop) }}" method="post">
        @endif
            @csrf
